...
class read_sensors:
    ...
    def __init__(self):
        ...
        self.save_data = []
        num_of_input = input('Enter number of sensors: ')
        self.TOTAL_SENSOR_NUM = int(num_of_input)
        ...
        for x in range(0,self.TOTAL_SENSOR_NUM):
            ...
            if ask == 'y':
                ...
                command = '0' + input('Insert ID (e.g:1): ') #'0%d' % (x+1)
            self.ser.write(command.encode())
            time.sleep(0.1)
            received = self.ser.readline()
            print(f'sensor_0{x+1}: ' + str(received))
